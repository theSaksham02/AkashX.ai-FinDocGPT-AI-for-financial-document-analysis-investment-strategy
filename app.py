...
def handle_api_request():    analysis_mode = st.selectbox(
    ...
    if analysis_mode == "💡 Investment Strategy":
        ...
        ticker = st.text_input("Stock Ticker (e.g., AAPL)", value="AAPL").upper()
        analysis_button = st.button("🔍 Analyze Stock", type="primary")

    elif analysis_mode == "🧠 AI Q&A System":
        question = st.text_input("Ask a question about a financial document")
...
if analysis_mode == "💡 Investment Strategy" and 'strategy_button' in locals() and strategy_button:
...
elif analysis_mode == "🔮 Financial Forecasting" and 'forecast_button' in locals() and forecast_button:
    if ticker:
        st.markdown(f"""
        <div class="feature-card">
...
elif analysis_mode == "📊 Stock Analysis" and 'analysis_button' in locals() and analysis_button:
    if ticker:
        st.markdown(f"""
        <div class="feature-card">
    ...
    if ticker:
        st.markdown(f"""
        <div class="feature-card">
            <h2>📈 Sentiment Analysis: {ticker}</h2>
        </div>
        """, unsafe_allow_html=True)
